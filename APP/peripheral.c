...
#define SBP_SCROLL_EVT_PERIOD              160
...
static uint8_t scroll_step_size = 2;
static uint8_t scroll_direction = 3;
static uint16_t word_color = 0xFFFF;


static uint8_t Peripheral_TaskID = INVALID_TASK_ID; // Task ID for internal task/event processing

// GAP - SCAN RSP data (max size = 31 bytes)
// 这里可以改外围设备被蓝牙扫描到时显示的名字，下面还有一部分需要修改，注意同时修改 length of this data，单位为字节，其他部分建议不要动
...
// GAP GATT Attributes
static uint8_t attDeviceName[GAP_DEVICE_NAME_LEN] = "Senyao Smart Screen"; // 要修改名字的话，这里也要修改
...
void Peripheral_Init()
{
    ...
    // Setup the SimpleProfile Characteristic Values
    {
    	...
    	uint8_t charValue2[SIMPLEPROFILE_CHAR2_LEN] = {0x03, 0x02, 0xFF, 0xFF};
        /*
    ...
    tmos_set_event(Peripheral_TaskID, SBP_SCROLL_UPDATE_EVT);
}
...
uint16_t Peripheral_ProcessEvent(uint8_t task_id, uint16_t events)
{
	...
	*/
    ...
    if(events & SBP_SCROLL_UPDATE_EVT)
        {
    		scroll_hanzi_display(BITMAP, scroll_direction, scroll_step_size, word_color, num_row, num_col);
    		tmos_start_task(Peripheral_TaskID, SBP_SCROLL_UPDATE_EVT, SBP_SCROLL_EVT_PERIOD);
            return (events ^ SBP_SCROLL_UPDATE_EVT);
        }
    /*
...
static void Peripheral_LinkEstablished(gapRoleEvent_t *pEvent)
{
    ...
    else
    {
        ...
        peripheralMTU = ATT_MAX_MTU_SIZE;
...
static void simpleProfileChangeCB(uint8_t paramID, uint8_t *pValue, uint16_t len)
{
    switch(paramID)
    {
        case SIMPLEPROFILE_CHAR1:
        {
            ...
            uint8_t hanzi_row = newValue[0];
            uint8_t hanzi_col = newValue[1];
            // 出现第一个字，说明需要推倒重来
            if (hanzi_row == 0 && hanzi_col == 0) {
            	num_row = 0;
            	num_col = 0;
        ...
        case SIMPLEPROFILE_CHAR2:
		{
			...
			tmos_memcpy(newValue, pValue, len);
			// 第 0 个为方向
			scroll_direction = newValue[0];
			// 第 1 个为步长
			scroll_step_size = newValue[1];
			// 第 2，3 为颜色，2 为高位，3 为低位
			word_color = newValue[2] << 8 | newValue[3];
			LCD_Clear(0x0000);
			// printf("dir=%d, step=%d\n", newValue[0], newValue[1]);
			// printf("2=%x, 3=%x,color=%x\n", newValue[2], newValue[3], word_color);
